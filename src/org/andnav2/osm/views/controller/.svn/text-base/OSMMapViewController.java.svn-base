...
package org.andnav2.osm.views.controller;
...
import org.andnav2.osm.views.util.MyMath;
 ...
 *
 */
public class OSMMapViewController {
	// ===========================================================
	// Constants
	// ===========================================================
	...
	private final OSMMapView mOsmv;
	...
	public OSMMapViewController(final OSMMapView osmv) {
		this.mOsmv = osmv;
	}
	...
	public void zoomToSpan(final int reqLatSpan, final int reqLonSpan) {
		...
		final int zoomLevelForSpan = getZoomLevelForSpan(reqLatSpan, reqLonSpan);
		if(zoomLevelForSpan != Constants.NOT_SET) {
			this.mOsmv.setZoomLevel(zoomLevelForSpan);
		}
	}

	public int getZoomLevelForSpan(final int reqLatSpan, final int reqLonSpan){
		...
		if(Float.isInfinite(diffNeededLat) || Float.isInfinite(diffNeededLon)) {
		...
		if(diffNeeded > 1){ // Zoom Out
			return curZoomLevel - MyMath.getNextSquareNumberAbove(diffNeeded);
		}else if(diffNeeded < 0.5){ // Can Zoom in
			return curZoomLevel + MyMath.getNextSquareNumberAbove(1 / diffNeeded) - 1;
		}else{
			return curZoomLevel;
		}
	}
	 ...
	 * Uses: {@link OSMMapViewController.ANIMATION_SMOOTHNESS_DEFAULT} and {@link OSMMapViewController.ANIMATION_DURATION_DEFAULT}.
	 * @param gp
	 */
	 ...
	 * @param aSmoothness steps made during animation. I.e.: {@link OSMMapViewController.ANIMATION_SMOOTHNESS_LOW}, {@link OSMMapViewController.ANIMATION_SMOOTHNESS_DEFAULT}, {@link OSMMapViewController.ANIMATION_SMOOTHNESS_HIGH}
	 * @param aDuration in Milliseconds. I.e.: {@link OSMMapViewController.ANIMATION_DURATION_SHORT}, {@link OSMMapViewController.ANIMATION_DURATION_DEFAULT}, {@link OSMMapViewController.ANIMATION_DURATION_LONG}
	 */
	...
	private abstract class AbstractAnimationRunner extends Thread{
		...
		public AbstractAnimationRunner(final OSMMapViewController openStreetMapViewController, final int aTargetLatitudeE6, final int aTargetLongitudeE6){
			this(aTargetLatitudeE6, aTargetLongitudeE6, OSMMapViewController.ANIMATION_SMOOTHNESS_DEFAULT, OSMMapViewController.ANIMATION_DURATION_DEFAULT);
		}
		...
		public AbstractAnimationRunner(final int aTargetLatitudeE6, final int aTargetLongitudeE6, final int aSmoothness, final int aDuration){
			...
			final OSMMapView mapview = OSMMapViewController.this.mOsmv;
